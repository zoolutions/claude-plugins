...
- **A flag assertion is anchored to its command and bounded.** `include("-p 8080 ")` matches `mkdir -p 8080` on another line and misses a correct `-p 8080` at the end of one. Anchor on the command name, stay on its line (`[^\n]*`), escape an interpolated value, and bound it with `(?=\s|$)`. When the value is read from a second file, compare with `.to_s` on both sides: YAML hands one file a String and the other an Integer for the same setting.
