...
- **Pair delimiters per line, never across a joined file.** Joining lines to catch a span wrapped at the column edge makes backtick (or quote) pairing file-global, so one line with an odd count inverts code and prose for the rest of the file: a real pointer after it is skipped and bare prose errors. Per-line pairing confines the damage; the wrapped span becomes the documented safe miss. Seen in getzazu/app#4011.
