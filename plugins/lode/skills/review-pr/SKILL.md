...
A merge commit already on the local branch and not yet pushed — `/lode:finish-prs` leaves a clean merge-forward this way — is this phase's work item too: gate it (the ledger's delta for a clean merge is empty, so the pass is immediate) and push.
