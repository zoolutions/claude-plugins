...
**Rigor** is the one heading the code cannot answer. Write `Default: standard` and an empty path table unless the user has stated a tier, and list "Rigor: set the default and name the money paths" as a maintainer decision in the PR body. A repo that leaves it at `standard` behaves exactly as before this heading existed.
