...
# append one bullet to plugins/lode/checklists/<matching file>.md, in that file's voice, with the repos it was seen in
git commit -am "learn: <rule> (seen in <repo>)" && git push -u origin HEAD
gh pr create --fill --body "Seen in <owner/repo>#<n>. <one paragraph: the failing case and the safe direction>."
```
