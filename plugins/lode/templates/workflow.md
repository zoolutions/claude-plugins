...
How much review a change here buys. `critical` is a money path: every gate agent plus a second correctness pass on concurrency, up to five rounds (each after the first on the delta of the last round's fixes), and no implementation without an issue or plan that carries a Decision section. `standard` is the default set. `light` is one round and the shorter ceremony in the five skills that read the tier (gate, lfg, review-pr, finish-prs, plan); the gate still skips agents with nothing to do, and still runs claims when the delta is prose. The push hook is the same at every tier.
