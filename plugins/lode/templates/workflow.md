...
A file no rule names counts as the default; the diff takes the highest tier over its files. A rule is a bash pattern against the repo-relative path: `*` spans `/`, `[...]` is a character class, a leading `./` or `/` is ignored. A rule ending in `/`, or naming a directory that exists, is a prefix; anything else is an exact path. Several patterns in one cell are comma-separated, so a pattern cannot contain a comma. Fenced code blocks are skipped. A row whose tier is not one of the three words is skipped with a note. `${CLAUDE_PLUGIN_ROOT}/scripts/rigor.sh` prints the tier; `--tier <t>` on a skill overrides it, and lowering needs a stated reason.
