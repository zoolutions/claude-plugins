...
1. **Shape × operation matrix.** From the tests, fixtures, docs and the existing code, list every input shape the changed code can meet (for a parser: every syntactic form; for a CLI: every option combination and every existing on-disk state; for a job: every ordering of concurrent actors). List every operation the diff touches. Walk the matrix cell by cell and ask what actually happens. Most real defects live in a cell nobody named: the remote pin with no comment, the subpath under a remote parent, the second process writing the same temp file.
2. **Siblings.** For every defect you find, look for the same shape elsewhere in the same file and in the files the diff did not touch but which share the regex, helper or invariant. Report the sibling as its own finding with its own line.
